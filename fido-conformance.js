 ...
 */
exports.fidoConformanceRouter.post('/attestation/options', async (req, res) => {
    const { body } = req;
    ...
    const opts = await (0, server_1.generateRegistrationOptions)({
        rpName,
        rpID: index_1.rpID,
        userID: helpers_1.isoBase64URL.toBuffer(username),
        userName: username,
 ...
 */
exports.fidoConformanceRouter.post('/assertion/options', async (req, res) => {
    const { body } = req;
    const { username, userVerification, extensions } = body;
    loggedInUsername = username;
    const user = inMemoryUserDB[username];
    const { credentials } = user;
    const opts = await (0, server_1.generateAuthenticationOptions)({
        rpID: index_1.rpID,
        extensions,
        userVerification,
